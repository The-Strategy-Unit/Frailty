...
SELECT 
      ...
      ,CASE
            WHEN [Der_Spell_LoS] = 0 THEN 'Discharged on admission date'
            WHEN [Discharge_Ready_Date] IS NULL THEN 'No DRD'
            WHEN [Discharge_Ready_Date] = [Discharge_Date] THEN 'Discharged on DRD'
            WHEN [Discharge_Date] > [Discharge_Ready_Date] THEN 'Discharged after DRD'
            WHEN [Discharge_Date] < [Discharge_Ready_Date] THEN 'Discharged before DRD'
       END AS [discharge_ready_status]
FROM [MESH_APC].[APCS_Core_Daily_1]
WHERE
-- include only age over 16
	[Age_At_CDS_Activity_Date] > 16
-- official definition says include only length of stay of >= 1 days 
-- but instead that is accounted for in the created variable
-- include only specific acute
AND	[Der_Dischg_Treatment_Function_Code] IN (100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 
                                             110, 111, 113, 115, 120, 130, 
                                             140, 141, 142, 143, 144, 145, 150, 160, 161, 
                                             170, 171, 172, 173, 174, 
                                             180, 190, 191, 192, 
                                             200, 211, 212, 213, 214, 215, 216, 217, 218, 219, 
                                             220, 221, 222, 230, 
                                             240, 241, 242, 
                                             250, 251, 252, 253, 254, 255, 256, 257, 258, 259, 
                                             260, 261, 262, 263, 264, 270, 280,
                                             300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 
                                             310, 311, 313, 314, 315, 316, 317, 318, 319, 
                                             320, 321, 322, 323, 324, 325, 326, 327, 328, 329, 
                                             330, 333, 335, 
                                             340, 341, 342, 343, 347, 348, 
                                             350, 352, 361, 370, 371, 
                                             400, 401, 410, 420, 421, 422, 
                                             430, 431, 450, 451, 460, 461, 
                                             502, 503, 505, 663, 670, 673, 675, 677, 
                                             800, 811, 812, 822, 834)
-- include only method of admission is elective or emergency (ie exclude 31,32,82,83)
AND [Admission_Method] NOT IN ('31', '32', '82', '83') 
),  apc_by_icd AS (
    SELECT
         [Der_Pseudo_NHS_Number]
        ,[Der_Diagnosis_All]
...
WHERE EXISTS (
    ...
    AND c2.[Admission_Date] BETWEEN DATEADD(year, -1, c1.[Admission_Date]) AND DATEADD(year, 1, c1.[Admission_Date])
    )
AND NOT ([Discharge_Destination] = '98')
GROUP BY 
       [Der_Activity_Month] 
      ,[GP_Practice_Code]
      ,[discharge_ready_status]
